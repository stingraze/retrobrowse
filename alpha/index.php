<?php   
/*   
 * retrobrowse version 0.11.4-a1 - for Ubuntu 20.04 & Windows Environments 
 * This version is intended for devices that don't have browsers with BASIC AUTH IMPLEMENTED. 
 * (C)Tsubasa Kato - 2024/7/17 22:21 PM JST - Created with help from ChatGPT (GPT-4o) & C:Amie  
 * Last updated on 2024/8/19 09:05 AM   
 ...
 */   
    ...
    # Paths 
    const IMG_ROOT_DIR		= 'C:\inetpub\wwwroot';			# POSIX systems use e.g. '/var/www/html/retrobrowse' 
									# Windows systems use e.g. 'C:\inetpub\wwwroot' 
    # Security 
    const AUTH_TYPE		= 'REQUIRE_AUTH_ANY';				# NONE | BASIC | FORMS | REQUIRE_AUTH_ANY 
    const AUTH_USERS_FILE	= 'C:\inetpub\passwords.txt';		# POSIX systems use e.g. '/var/www/html/passwords.txt' 
									# Windows systems use e.g. 'C:\inetpub\passwords.txt' 
									# Do NOT place the file under the wwwroot / public_html folder! 
...
function challengeAuthentication(): void { 
    ...
    switch (AUTH_TYPE) { 
        case 'NONE': 
            return; 
        case 'BASIC':										// BASIC & REQIRE_AUTH_ANY 
        case 'REQUIRE_AUTH_ANY': 
            /* test for username/password */ 
            if (!( 
                    isset($_SERVER['PHP_AUTH_USER']) && isset($_SERVER['PHP_AUTH_PW']) && 
                    testCredentials($_SERVER['PHP_AUTH_USER'], $_SERVER['PHP_AUTH_PW']) 
                 )) 
            { 
            ...
            if (AUTH_TYPE === 'BASIC') {								// REQUIRE_AUTH_ANY can cascade to the next eval group 
                // Show failure text, which browsers usually 
                // show only after several failed attempts 
                print("Access denied.<br>\n"); 
                exit(0); 
            } 
        case 'FORMS':										// FORMS & REQUIRE_AUTH_ANY 
        case 'REQUIRE_AUTH_ANY': 
	    ...
	    <?php 
            ...
            break; 
        default: 
            print("Security configuration error.<br>\n"); 
            exit(0); 
    } 
}  
 
function testCredentials($username, $password) { 
    $txtF = fopen(AUTH_USERS_FILE, 'r'); 
    while ($line = fgets($txtF)) { 
      $arrParts = explode('	', $line); 
      if (count($arrParts) >= 2) { 
        if (($arrParts[0] === $username) && ($arrParts[1] === $password)) { 
          return true; 
        } 
      } 
    } 
